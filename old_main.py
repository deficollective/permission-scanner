...
def main():
    ...
    for contract_address in contracts_addresses:
        contract_result = fetch_contract_metadata(
        ...
        implementation_name = ""
        contract_data_for_markdown.append(
            {"name": contract_name, "address": contract_address}
        )

        if isProxy and implementation_address:
            if not isinstance(implementation_address, str) or not is_valid_eth_address(
        ...
        try:
            ...
            print(
                f"\033[33mFailed to compile contract at {contract_address} due to HTTP error: {e}\033[0m"
            )
            continue  # Skip this contract and move to the next one
        except Exception as e:
            print(
                f"\033[33mAn error occurred while analyzing {contract_address}: {e}\033[0m"
            )
            continue
        ...
        target_contract = [
            contract for contract in contracts if contract.name == contract_name
        ]
        ...
        if isProxy:
            ...
            target_contract.extend(
                [
                    ...
                    if contract.name == implementation_name
                ]
            )
                        ...
                        if var.name in functionData["state_variables_read"]:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    value = srs.convert_value_to_type(
