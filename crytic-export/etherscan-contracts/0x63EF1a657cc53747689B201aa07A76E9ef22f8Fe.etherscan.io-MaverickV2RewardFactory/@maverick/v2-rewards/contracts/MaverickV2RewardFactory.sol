...
contract MaverickV2RewardFactory is IMaverickV2RewardFactory {
    ...
    function createRewardsContract(
        ...
        IERC20 lastRewardToken;
        for (uint256 k; k < length; k++) {
            if (lastRewardToken >= rewardTokens[k])
                revert RewardFactoryTokensNotSorted(k, lastRewardToken, rewardTokens[k]);
            lastRewardToken = rewardTokens[k];
